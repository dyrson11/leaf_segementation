...
kernel_open =  np.copy(cv2.getStructuringElement(cv2.MORPH_RECT,(10,10)))
kernel_close =  np.copy(cv2.getStructuringElement(cv2.MORPH_RECT,(400,400)))
...
def segment_leaf(image_file, filling_mode, smooth_boundary, marker_intensity):
    ...
    bin_image = cv2.morphologyEx(bin_image, cv2.MORPH_OPEN, kernel_open)
    bin_image = cv2.morphologyEx(bin_image, cv2.MORPH_CLOSE, kernel_close)
    ...
    mask = cv2.inRange(image, (0, 40, 60), (96, 255, 255))
    image = cv2.cvtColor(image, cv2.COLOR_HSV2BGR)
    mask = mask.astype(np.uint8)

    n_image = np.zeros_like(image, np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel_open)
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel_close)
    imask = mask > 0

    n_image[imask] = image[imask]

    return original, n_image
...
def enhance_image(path):
    ...
    image = cv2.cvtColor(hsv_eq, cv2.COLOR_HSV2BGR)
    ...
    n_image = np.zeros_like(image, np.uint8)
    n_image[marker] = original_image[marker]
    image = n_image

    # b, g, r = cv2.split(image)
    # # Applying equalize Hist operation on Y channel.
    # g_eq = cv2.equalizeHist(g)
    # image = cv2.merge((b, g_eq, r))
    ...
    image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    mask = cv2.inRange(image, (12, 0, 100), (86, 255, 255))
    image = cv2.cvtColor(image, cv2.COLOR_HSV2BGR)
    mask = mask.astype(np.uint8)
    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel_open)
    ...
    largest_mask = \
        select_largest_obj(mask, fill_mode=filling_mode,
                           smooth_boundary=False)

    imask = largest_mask > 0

    n_image[imask] = original_image[imask]
...
if __name__ == '__main__':
    ...
    for file in files:
        try:
            ...
            output_image = enhance_image(os.path.join(base_folder, file))
        except ValueError as err:
...
# python main.py -f no -d Data\segmentado "Data\A10\Aniba rosaeodora"
